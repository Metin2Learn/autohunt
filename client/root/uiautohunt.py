...
class AutoHunt(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		try:
			...
			self.editlines = []
			for i in xrange(16):
				editLine = GetObject("editline%s" % (str(i)))
				editLine.OnPressEscapeKey = ui.__mem_func__(self.Close)
	...
	def Destroy(self):
		self.board = None
		self.Auto_Active_Skill_Slot_Table = None
		self.AutoSkillClearButton = None
		self.AutoPositionClearButton = None
		self.AutoAllClearButton = None
		self.AutoAttackOffButton = None
		self.AutoAttackOnButton = None
		self.AutoSkillOffButton = None
		self.AutoSkillOnButton = None
		self.AutoPositionlOffButton = None
		self.AutoPositionlOnButton = None
		self.AutoStartOffButton = None
		self.AutoStartOnButton = None
	...
	def SaveSettings(self):
		...
		hs = open("UserData/"+str(player.GetName()),"w")
		for i in xrange(16):
	...
	def LoadSettings(self):
		if os.path.exists("UserData/"+str(player.GetName())):
			playerSettings = open("UserData/"+str(player.GetName()), "r")
			playerSettingsList = playerSettings.read()
			playerSettingLine = playerSettingsList.split("\n")
			playerSettings.close()
			for i in xrange(str(playerSettingsList).count("\n")):
				lin = str(playerSettingLine[i]).split("\t")
				if int(lin[0]) != -1:
					if i < 8:
